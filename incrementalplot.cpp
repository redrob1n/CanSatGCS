...
void IncrementalPlot::showSymbols( bool on )
{
    if ( on )
    {
        d_curve->setStyle( QwtPlotCurve::Lines ); //sets style of plot to a line, can be change to Dots or no curve at all
        d_curve->setPen(Qt::blue, 2);
        /*d_curve->setSymbol( new QwtSymbol( QwtSymbol::Ellipse,
            Qt::NoBrush, QPen( Qt::blue ), QSize( 4, 4 ) ) );
            */
        d_curve->setSymbol( new QwtSymbol( QwtSymbol::Ellipse,
                    QBrush(Qt::blue), QPen( Qt::blue ), QSize( 4, 4 ) ) );
    }
    else
    {
